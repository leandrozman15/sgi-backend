...
export class CarriersService {
  ...
  async getById(req: { companyId: string }, id: string) {
    const row = await this.prisma.carrier.findFirst({
  ...
  async create(req: { companyId: string }, dto: any) {
    ...
    const row = await this.prisma.carrier.create({
      data: {
        ...
        nome: nome || "Transportadora Sem Nome",
        data: rest,
      },
    });
    return this.toPublic(row);
  ...
  async update(req: { companyId: string }, id: string, dto: any) {
    ...
    const row = await this.prisma.carrier.update({
      where: { id },
      data: {
        nome: nome ?? undefined,
        data: finalData,
  ...
  async remove(req: { companyId: string }, id: string) {
    ...
    if (result.count === 0) throw new NotFoundException("Transportadora não encontrada.");
    return { ok: true };
  }
}
