import { Controller, Get, Post, Body, Param, Patch, Delete, UseGuards } from '@nestjs/common';
import { SubscriptionHistoryService } from './subscriptions.service';
import { FirebaseAuthGuard } from '../auth/firebase-auth.guard';
import { TenantMembershipGuard } from '../auth/guards/tenant-membership.guard';
import { Tenant } from '../auth/decorators/tenant.decorator';

@Controller('subscriptions')
@UseGuards(FirebaseAuthGuard, TenantMembershipGuard)
export class SubscriptionHistoryController {
  constructor(private readonly subscriptionhistoryService: SubscriptionHistoryService) {}

  @Get()
  findAll(@Tenant('companyId') companyId: string) {
    return this.subscriptionhistoryService.findAll(companyId);
  }

  @Get(':id')
  findOne(@Param('id') id: string, @Tenant('companyId') companyId: string) {
    return this.subscriptionhistoryService.findOne(id, companyId);
  }

  @Post()
  create(@Body() createDto: any, @Tenant('companyId') companyId: string) {
    return this.subscriptionhistoryService.create(createDto, companyId);
  }

  @Patch(':id')
  update(@Param('id') id: string, @Body() updateDto: any, @Tenant('companyId') companyId: string) {
    return this.subscriptionhistoryService.update(id, updateDto, companyId);
  }

  @Delete(':id')
  remove(@Param('id') id: string, @Tenant('companyId') companyId: string) {
    return this.subscriptionhistoryService.remove(id, companyId);
  }
}
